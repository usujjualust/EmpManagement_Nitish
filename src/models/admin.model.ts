import { BaseEntity, Column, CreateDateColumn, Entity, PrimaryGeneratedColumn, Like, Equal } from "typeorm";
import { UserRegistry } from "./user.model";
import { AppDataSource } from "../db";

export type Admin_Level = 
    | 'limited'
    | 'super'
...
export class AdminTable extends BaseEntity{
    ...
    static async LoadAdminsFromUserRegistry():Promise<void>{
        ...
        const admins:UserRegistry[] = await AppDataSource.getRepository(UserRegistry)
                                        .findBy({
                                                 user_id: Like("USR%"),
                                                 role: Equal("admin")
                                                })
       ...
       for(const admin of admins){
        ...
        const existingAdmin = await AdminTable.findOne({where: {admin_id: user_id}})
