...
const GetAdmin = asyncHandler(async(req:Request, res:Response, next:NextFunction)=>{
    try {
        ...
        const admins: Admin[] = await AdminTable.createQueryBuilder()
                                                .getMany()
        //  res.send('ok')                                     
        if(admins.length === 0){  
...
const GetAll = asyncHandler(async(req:Request, res:Response, next:NextFunction)=>{
   try {
    const users: User[] = await UserRegistry.createQueryBuilder('user')
                                .getMany();
    if(users.length === 0){
        return res.status(404).send("Empty, no data found!")
    }else{
...
const DeleteUser = asyncHandler(async(req: Request <Array<{user_id?: string, email?:string}>>, res: Response, next: NextFunction)=>{
    try {
    ...
    if(foundUser === null){
               ...
               return res.status(404).send(`Reaquired user ${user.user_id} ${user.email} not found`).statusMessage = `User not found`;
    }
